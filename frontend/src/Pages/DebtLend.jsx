...
export default function DebtLend() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        const res = await fetch("http://localhost:5000/api/debtlend", {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/debtlend/${id}/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/debtlend/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
